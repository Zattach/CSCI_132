...
class IMP implements MouseListener{
  ...
  private void fun2()
  {
	  ...
	  Stack<PixelRegion> s = new Stack();
	  
      int rgbArray[] = new int[4];
      rgbArray = getPixelArray(picture[colorY][colorX]);
      
      PixelRegion p = new PixelRegion(colorY, colorX);
      s.push(p);
      ...
      while(!s.empty()){
    	 ...
    	 PixelRegion l = s.pop();
    	  ...
    	  if(l.getX()-1 >= 0 && l.getY()-1 >= 0 && l.getX()+1 < width && l.getY()+1 < height){
    		  if(!painted[l.getY()][l.getX()]){
    			  
    			  if(getPixels(getPixelArray(picture[l.getY()][l.getX()])) == getPixels(getPixelArray(picture[l.getY()+1][l.getX()+1]))){
    				  s.push(new PixelRegion(l.getY()+1, l.getX()+1));
    			  }
    			  if(getPixels(getPixelArray(picture[l.getY()][l.getX()])) == getPixels(getPixelArray(picture[l.getY()+1][l.getX()]))){
        			  s.push(new PixelRegion(l.getY()+1, l.getX()));
    			  }
    			  if(getPixels(getPixelArray(picture[l.getY()][l.getX()])) == getPixels(getPixelArray(picture[l.getY()+1][l.getX()-1]))){
    				  s.push(new PixelRegion(l.getY()+1, l.getX()-1));
    			  }
    			  if(getPixels(getPixelArray(picture[l.getY()][l.getX()])) == getPixels(getPixelArray(picture[l.getY()][l.getX()+1]))){
    				  s.push(new PixelRegion(l.getY(), l.getX()+1));
    			  }
    			  if(getPixels(getPixelArray(picture[l.getY()][l.getX()])) == getPixels(getPixelArray(picture[l.getY()][l.getX()-1]))){
    				  s.push(new PixelRegion(l.getY(), l.getX()-1));
    			  }
    			  ...
    			  picture[l.getY()][l.getX()] = getPixels(rgbArray);
    			  painted[l.getY()][l.getX()] = true;
    		  }
    	  }
    	  
    	  
    	  
    	  /*if(getPixels(getPixelArray(picture[p.getY()][p.getX()])) == getPixels(getPixelArray(picture[p.getY()-1][p.getX()-1]))){
    		  int j = p.getX();
    		  int i = p.getY();
    		  for(i=p.getY(); i>0 && getPixels(getPixelArray(picture[i][j])) == getPixels(getPixelArray(picture[i-1][j])); i--){
    			  for(j=p.getX(); j>0 && getPixels(getPixelArray(picture[i][j])) == getPixels(getPixelArray(picture[i][j-1])); j--){
    	  ...
    	  if(getPixels(getPixelArray(picture[p.getY()][p.getX()])) == getPixels(getPixelArray(picture[p.getY()-1][p.getX()-1]))){
    		  ...
    		  for(int i = colorY;i > 0; i--){
    			  for(int j = colorX; j > 0; j--){
    			  }
    		  }
    	  }*/
      }
